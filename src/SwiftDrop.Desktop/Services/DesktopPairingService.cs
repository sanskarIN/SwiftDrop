...
public sealed class DesktopPairingService
{
    private readonly DesktopIdentityService _identity;

    public DesktopPairingService(DesktopIdentityService identity)
    {
        _identity = identity;
    }

    public async Task<PairingPayload> RequestAsync(PeerDevice peer, string? pairingCode = null, CancellationToken ct = default)
    {
        ArgumentNullException.ThrowIfNull(peer);
        if (string.IsNullOrWhiteSpace(peer.CertificateFingerprint))
            throw new InvalidOperationException("The discovered device did not advertise a certificate fingerprint. Use pairing-link or manual-IP pairing instead.");
        ...
        await using var stream = await client.ConnectAsync(host, peer.Port, expectedFingerprint, _identity.Certificate, ct);
