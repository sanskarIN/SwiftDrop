...
public sealed class DesktopDiscoveryService : IAsyncDisposable
{
    ...
    public DesktopDiscoveryService(DesktopIdentityService identity)
    {
        _identity = identity;
    }
    ...
    public async Task StartAsync(CancellationToken ct = default)
    {
        ...
        {
            try { await _mdns.QueryAsync(token); }
    ...
    private async Task AnnounceLoopAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            if (_mdns is not null)
            {
                try { await _mdns.AnnounceAsync(self, ct); }
                catch (Exception ex) when (ex is SocketException or InvalidOperationException) { }
            }
            if (_udp is not null)
            {
                try { await _udp.AnnounceAsync(self, ct); }
                catch (Exception ex) when (ex is SocketException or InvalidOperationException) { }
            }
